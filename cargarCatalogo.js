...
async function sincronizar() {
  ...
  console.log("SINCRONIZANDO CATALOGO HISTORICO...");

  for (const product_id in CATALOGO) {

    const item = CATALOGO[product_id];

    await pool.query(`
      INSERT INTO catalogo (product_id, variant_id, nombre, area, url, activo)
      VALUES ($1,$2,$3,$4,$5,true)
      ON CONFLICT DO NOTHING
    `,[product_id,item.variant,item.nombre,item.area,item.url]);

    await pool.query(`
      UPDATE catalogo
      SET 
        variant_id = COALESCE(variant_id,$1),
        nombre = COALESCE(nombre,$2),
        area = COALESCE(area,$3),
        url = COALESCE(url,$4)
      WHERE product_id=$5
    `,[item.variant,item.nombre,item.area,item.url,product_id]);
  }

  console.log("CATALOGO SINCRONIZADO OK");
  process.exit();
}
