...
import rateLimit from "express-rate-limit"; // ✅ FIX CRÍTICO

// 🔴 MANTENIDOS COMENTADOS (no romper deploy)
...
app.get("/dashboard/master", async (req, res) => {
  ...
  try {
    ...
    <body>
      ...
      <div class="card">
        <h3>Resumen</h3>
        <p>Usuarios: ${Number(totalUsers.rows[0].count)}</p>
        <p>Feedback total: ${Number(totalFeedback.rows[0].count)}</p>
      </div>
...
/* 🔴 SEARCH_CATALOG SE MANTIENE EXACTO (NO SE MODIFICA) */
...
app.get("/api/search", (req, res) => {
  try {
    ...
    const results = SEARCH_CATALOG.map(item => {
      ...
      if (nombre === q) score += 30;
      else if (nombre.startsWith(q)) score += 20;
      else if (q.length > 4 && nombre.includes(q)) score += 10;

      for (const kw of item.keywords) {
        if (kw === q) score += 12;
        else if (kw.startsWith(q)) score += 6;
        else if (q.length > 5 && kw.includes(q)) score += 2;
      }

      if (item.area === "academy") score += 3;
...
console.error("ERROR AUDIT GET:", error);
...
/* =========================================================
36 - VALIDACIÓN TOKEN (FIX DEPLOY)
========================================================= */
...
app.post("/api/validate-token", async (req, res) => {
  try {
    ...
    // 🔥 SOLO COLUMNAS SEGURAS (evita crash si faltan campos nuevos)
    const result = await pool.query(`
      SELECT
        token,
        email,
...
/* =========================================================
37 - GUARDAR FEEDBACK DEL ESTUDIANTE
========================================================= */
...
/* =========================================================
🧠 CHAT POST (PRODUCCIÓN REAL - COMPLETO + BLINDADO)
========================================================= */
...
app.post("/api/chat", chatLimiter, async (req, res) => {
  ...
  try {
      ...
      if (rating <= 3) {
        ...
        if (Object.keys(pref).length > 0) {
          try {
            ...
            console.warn("⚠ user_preferences pendiente:", err.message);
          }
        }
      }
    ...
    try {
      ...
      console.warn("⚠ tutor_config pendiente:", err.message);
    }
    ...
    try {
      ...
      if (prefResult.rowCount) {
        prefs = prefResult.rows[0].preferences;
      }
    } catch (err) {
      console.warn("⚠ prefs no disponibles");
    }

    /* =========================================================
    🧠 STABILITY ENGINE (COMPLETO)
    ========================================================= */
    ...
    try {
      ...
      console.warn("⚠ user_behavior no disponible");
    }
    ...
    try {
      ...
      console.warn("⚠ user_behavior write pendiente");
    }
    ...
    function generateResponse(message, prefs, config) {
      ...
      if (config.max_questions > 2) {
        response += "\n\n¿Esto lo estás aplicando actualmente?";
      }

      if (config.pacing_level <= 2) {
        response += "\n\nVamos con calma para evitar confusión.";
      }
...
/* =========================================================
🚀 START SERVER (CRÍTICO PARA DEPLOY)
========================================================= */

const PORT = process.env.PORT || 3000;

/* 🔥 FIX IMPORTANTE PARA RAILWAY / PRODUCCIÓN */
...
/* =========================================================
🛑 MANEJO GLOBAL DE ERRORES (ANTI-CRASH FINAL)
========================================================= */
