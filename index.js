...
async function enviarCorreo(destino, producto, token) {
  try {
    ...
    if(!destino){
      console.error("EMAIL VACÍO — no se envía");
      return false;
    }
...
324464294:{variant:1438540878,area:"tutor",nombre:"TAP Contaduría",url:"https://chatgpt.com/g/g-69684f74a91c8191850a3f43493f2c78-tap-de-contaduria-accounting-pat"}
...
app.post("/webhooks/tiendanube/order-paid", async (req, res) => {
  ...
  try {
    ...
    if(!orderId){
      console.error("WEBHOOK SIN ORDER ID");
    ...
    if(!curso){
      console.error("PRODUCTO FUERA DE CATALOGO:",productId,variantId);
      return res.sendStatus(500);
    }
    ...
    const enviado = await enviarCorreo(email, curso, token);
