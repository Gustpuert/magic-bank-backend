...
/* =========================
   WEBHOOK ORDER PAID (FIX PRODUCCIÓN)
========================= */
app.post("/webhooks/tiendanube/order-paid", (req, res) => {

  // RESPUESTA INMEDIATA A TIENDANUBE
  ...
  setImmediate(async () => {
    try {
      ...
      console.log("AUTOMATIZACION OK:", email, variantId);

    } catch (err) {
      console.error("WEBHOOK BACKGROUND ERROR:", err.response?.data || err.message);
    }
  });
});
