...
app.post("/webhooks/tiendanube/order-paid", async (req, res) => {
  try {
    ...
    if (order.data.payment_status !== "paid") {
      console.log("⏳ Pago no confirmado aún");
    ...
    await pool.query(
      ...
      (token, email, product_id, product_name, area, redirect_url)
      VALUES ($1,$2,$3,$4,$5,$6)`,
...
app.get("/access/:token", async (req, res) => {
  const { rows } = await pool.query(
    "SELECT redirect_url FROM access_tokens WHERE token=$1",
    [req.params.token]
  );

  if (!rows.length) return res.status(403).send("Acceso inválido");
