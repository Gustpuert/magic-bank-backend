...
app.get("/setup-webhook", async (_, res) => {
  try {
    ...
    if (!store.rowCount) return res.status(400).send("No hay tienda conectada");
...
async function procesarOrden(orderId) {
  try {
    ...
    console.log("ENTREGA OK:", email);

  } catch (err) {
    console.error("ERROR PROCESAR ORDEN:", err.message);
  }
}
