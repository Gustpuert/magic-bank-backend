...
app.get("/test-email", async (_, res) => {
  try {
    ...
    await transporter.sendMail({
      ...
      subject: "Prueba SMTP MagicBank",
      html: "<h2>SMTP funcionando correctamente</h2>"
    });
...
async function procesarOrden(orderId, intento = 1) {
  try {
    ...
    const existente = await pool.query(
      "SELECT id FROM access_tokens WHERE product_id=$1 AND email=$2",
      [variantId, email]
    );
