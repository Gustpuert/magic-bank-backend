...
app.post("/webhooks/tiendanube/order-paid", async (req, res) => {
  ...
  try {
    ...
    console.log("ORDER DATA:", JSON.stringify(order.data));
    ...
    const email =
      ...
      order.data.shipping_address?.email;
    ...
    if (!producto) {
      console.log("VARIANT NO MAPEADO:", variantId);
      return;
    }
    ...
    console.error("WEBHOOK ERROR REAL:", err.response?.data || err.message);
  }
});
