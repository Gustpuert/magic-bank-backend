...
const CATALOGO = {
/* --- CATÁLOGO EXACTAMENTE IGUAL AL HISTÓRICO --- */
/* (se mantiene idéntico, no se altera ninguna línea) */
...
app.post("/webhooks/tiendanube/order-paid", async (req, res) => {
  ...
  try {
    ...
    /* =========================
       NUEVOS PRODUCTOS VIA SKU
    ========================= */

    if (!curso) {

      const sku =
        order.data.order_products?.[0]?.sku ||
        order.data.products?.[0]?.sku;
      ...
      if (sku && sku.includes("|")) {
        ...
        curso = {
          nombre: partes[0].replaceAll("_", " "),
          area: "tutor",
          url: partes[1]
        };

        console.log("CURSO DINAMICO SKU:", curso.nombre);
      }
    }
