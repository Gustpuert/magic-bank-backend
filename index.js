...
app.get("/analytics", async (_, res) => {
  try {
    ...
    const totalAlumnos = await pool.query(`SELECT COUNT(*) FROM access_tokens`);
...
app.post("/webhooks/tiendanube/order-paid", async (req, res) => {
  ...
  try {
    ...
    const order=await axios.get(
      ...
      {headers:{Authentication:`bearer ${access_token}`,"User-Agent":"MagicBank"}}
    );

    if(order.data.payment_status!=="paid") return;

    const item = order.data.order_products?.[0];
    ...
    if(!curso){
      console.log("NO EN CATALOGO:",productId);
      return;
    }
